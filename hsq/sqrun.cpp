...
bool loadfrom(istream & in)
{
    while (1)
    {
        ...
        if ( s[0] == '#' )
        {
            ...
            else if ( s.size() == s_in.size() + 1 && s.substr(1) == s_in ) mem[ip++] = iin;
            else
            {
...
int main(int ac, char * av[])
{
    ...
    for ( int i = 1; i < ac; i++ )
    {
        ...
        else if ( s.size() > 9 && s.substr(1, 8) == "outaddr=" ) iout = atoi(s.substr(9).c_str());
        else if ( s.size() > 8 && s.substr(1, 7) == "inname=" ) s_in = s.substr(8);
        else if ( s.size() > 8 && s.substr(1, 7) == "inaddr=" ) iin = atoi(s.substr(8).c_str());
        else if ( s.size() == 6 && s.substr(1, 5) == "trace" ) trace = true;
        else if ( s.size() == 6 && s.substr(1, 5) == "steps" ) steps = true;
        else if ( s.size() == 6 && s.substr(1, 5) == "stdin" ) bcin = true;
        else if ( s.size() == 2 && s.substr(1, 1) == "a" ) addleq = true;
        else if ( s.size() == 2 && s.substr(1, 1) == "p" ) p1eq = true;
