...
void dumpblock(ofstream & out, ifstream & inp,
               unsigned long block, unsigned long nblock)
{
...
string constructname(const char * base,
                     unsigned long block, unsigned long num)
{
    ...
    sb << block;
    sn << num;
    name += string(".") + sb.str() + "_" + sn.str() + ".rez";
    return name;
...
//  string block_name(getname(av[1]));
//  block_name += string(".") + av[2] + "_" +av[3] +".rez";

    string block_name = constructname(av[1], block, nblock);
    ...
    dumpblock(out, inp, block, nblock);

    return 0;
}


bool ismember(list<unsigned long> & l, unsigned long n)
{
...
int rez_sub(int ac, char * av[])
{
    ...
    if ( ac < 4 )
    {
        cerr << "Use: subst <filename> block_size block_number(s)\n"
             "e.g. subst a.exe 100 1234\n"
             "or subst a.exe 100 \"1234 23 567 1\"\n";
        return 1;
    }
...

    {
        ...
        while (1)
        {
            ...
            lblock.push_back(nblock);
        }

    }

//*******************
    string out_name(getname(av[1]));
    ...
    list<unsigned long>::iterator lit;
    cout << "Blocks to substitute: ";
    for ( lit = lblock.begin(); lit != lblock.end(); ++lit )
    {
        string block_name = constructname(av[1], block, *lit);
...
//*******************
    ...
    while ( !!inp )
    {
        ...
        if ( ismember(lblock, ++num) )
        {
            {
                string block_name = constructname(av[1], block, num);
                ifstream bin(block_name.c_str(), ios::binary);
                dumpblock(out, bin, block, 1);
            }
            cout << '[' << num << "] ";
        }
        else
        {
            dumpblock(out, inp, block, num);
            cout << num << ' ';
        }
