...
$cook=$_COOKIE["last2leave23"];
if( $cook == "" )
{
    if( !empty($_REQUEST) && $_REQUEST["wget"] != "" )
    {
        $cook = $_REQUEST["wget"];
    }
    else
    {
    $cook = uniqid();
    setcookie("last2leave23",$cook,time()+60*60*24*3000);
    if( !empty($_REQUEST) && $_REQUEST["nocook"] != "" ) die("Please enable cookies");
    die("<a href='index.php?nocook=yes'>Start using this tool!</a>");
    }
}
...
echo " cook=[".$_COOKIE["last2leave23"]."]";
echo "<br/>";
}
//dbug();
...
<small>To add name: type your name in the text field and select time (do not click "Reload")
<br/>
To remove name: type "delete" or "delete <i>name</i>", e.g. delete Frans de Waal
<br/>

<?php
echo "".date('d/m/Y h:i:s a', time()). ", v2.0";
?>
